...
from carrie_interfaces.action import Move, Park
...
class CommandReceiver(Node):
	def __init__(self, comms, vicon_receiver):
		...
		self.move_action = ActionServer(
			...
			cancel_callback=self.stop_callback)

		self.park_action = ActionServer(
			self,
			Park,
			'/carrie/parking',
			execute_callback=self.park_callback,
			cancel_callback=self.stop_callback)
	...
	def move_callback(self, goal):
		self.canceled = False
		time.sleep(1) # XXX: why do we do this?
		...
		elapsed_time = -1.0
		try:
			error, elapsed_time = self.movement(end_x, end_y, end_yaw, velocity, goal, feedback)
		except Exception as e:
			self.get_logger().error('miss reported!')
			traceback.print_exc()

		self.comment_on_move(goal, error, tolerance)

		result.error = error
		result.elapsed_time = elapsed_time
		return result

	def park_callback(self, goal):
		self.canceled = False
		time.sleep(1) # XXX
		feedback = Park.Feedback()
		...
		# do the first move, to line up with the lead barrel
		velocity = .35
		spot_yaw = spot.get_goal_orientation()
		lat_dist = .35 # m to the right of barrel_2
		lead_dist = 1.2 # m ahead of the spot

		end_x = barrel_1.x + lead_dist*np.cos(spot_yaw) + lat_dist*np.sin(spot_yaw)
		end_y = barrel_1.y + lead_dist*np.sin(spot_yaw) - lat_dist*np.cos(spot_yaw)
		end_yaw = spot_yaw

		self.get_logger().info('lining up with the spot...')
		error, _ = self.movement(end_x, end_y, end_yaw, velocity, goal, feedback)
		if not self.canceled:
			self.get_logger().info('lined up. taking a breather.')
			time.sleep(2)

			# do the second move, backing in
			velocity = -.2
			ratio = .28
			end_x = barrel_1.x + ratio*(barrel_2.x - barrel_1.x)
			end_y = barrel_1.y + ratio*(barrel_2.y - barrel_1.y)

			self.get_logger().info('backing in...')
			error, _ = self.movement(end_x, end_y, end_yaw, velocity, goal, feedback)

		if not self.canceled:
			self.get_logger().info('backed in. taking a shorter breather.')
			time.sleep(1)

			# do the third move to finish 'er off
			velocity = .2
			pull_ahead = .06 # XXX
			midpoint = spot.get_goal_point()
			end_x = midpoint.x + pull_ahead*np.cos(spot_yaw)
			end_y = midpoint.y + pull_ahead*np.sin(spot_yaw)

			self.get_logger().info('finishing up...')
			error, _ = self.movement(end_x, end_y, end_yaw, velocity, goal, feedback)
			self.get_logger().info('should be finished?')
	...
	def comment_on_move(self, goal, error, tolerance):
		if error <= tolerance:
			if not self.canceled:
				goal.succeed()
	...
	def movement(self, end_x, end_y, end_yaw, velocity, goal, feedback):
		wheelbase = .22 # XXX

		lead_dist = math.copysign(wheelbase/2, velocity)
...
#		self.get_logger().debug(f'\n\tpath_x={path_x}\n\tpath_y={path_y}\n\tpath_yaw={path_yaw}\n')
		...
		outbound_id = 1
		inbound_id = 0
		while not self.canceled and math.dist((rx, ry), (end_x, end_y)) > tolerance:
...
#			print(f'rx={rx} ry={ry} dist={np.hypot(end_x - rx, end_y - ry)}')
		...
		error = math.dist((rx, ry), (end_x, end_y))
